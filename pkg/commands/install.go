...
// Outer key: package manager (Homebrew, APT, YUM, Scoop, Chocolatey, Pacman)
// Inner key: tool name (git, docker, lazygit, lazydocker, etc.)
// Value: shell command to install the tool
//
// Standard package manager commands are auto-generated using helper functions.
// Curl commands remain hardcoded due to their complex nature.
var PackageManagerInstallCommands = LifecycleCommandsType{
	"Homebrew":   buildToolMap("Homebrew", "install"),
	"APT":        buildToolMap("APT", "install"),
	"YUM":        buildToolMap("YUM", "install"),
	"DNF":        buildToolMap("DNF", "install"),
	"Nix":        buildToolMap("Nix", "install"),
	"Scoop":      buildToolMap("Scoop", "install"),
	"Chocolatey": buildToolMap("Chocolatey", "install"),
	"Pacman":     buildToolMap("Pacman", "install"),
	"Curl": {
		...
		"codex":       "arch=$(uname -m); case \"$arch\" in x86_64) arch=\"x86_64\" ;; arm64|aarch64) arch=\"aarch64\" ;; *) echo \"Unsupported architecture: $arch\" && exit 1 ;; esac && curl -fsSL https://github.com/openai/codex/releases/latest/download/codex-$arch-unknown-linux-musl.tar.gz -o /tmp/codex.tar.gz && cd /tmp && tar -xzf codex.tar.gz && chmod +x codex-* && sudo mv codex-* /usr/local/bin/codex",
		"openclaw":    openclawUnixInstallScript,
...
// buildToolMap creates a map of tool names to commands for a specific package manager
func buildToolMap(method, commandType string) map[string]string {
	...
	for _, tool := range tools.Tools {
		...
		switch commandType {
		case "install":
			cmd = GenerateInstallCommand(method, tool)
		case "update":
			cmd = GenerateUpdateCommand(method, tool)
		case "uninstall":
			cmd = GenerateUninstallCommand(method, tool)
		}
