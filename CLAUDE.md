...
GOOS=linux GOARCH=amd64 go build -o lazysetup-linux-amd64
```
...
- Three state files: `state.go` (struct), `state_methods.go` (accessors), `state_installation.go` (installation state)
...
**pkg/handlers/**
- `handlers_navigation.go`: Tab/arrow key navigation, panel switching
...
**pkg/ui/**
- `layout_multipanel.go`: Main 4-panel layout orchestrator
- `layout_panels.go`: Individual panel renderers (Package Manager, Action, Tools)
- `keybindings.go`: Centralized keybinding registration
